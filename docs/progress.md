...
- **Sessions** (2026-07-13): implemented over the mesh. One roster per session (leave frees the seat); active-session handle is a live view, not a snapshot. Search reply packs all of a peer's matches into one response so none are dropped. Inbound `session_join_response` is accepted only from the session's owner, and a verdict about us acts only while a join we started is pending — a forged verdict cannot erase a session or inject ghosts. Wire list lengths are bounded to the ABI maxima before reserve (a hostile length otherwise reserves 100s of MB / crashes across the C ABI).
- **Presence** (2026-07-13): implemented over the mesh, keyed by Epic account id. Own presence seeded at init and broadcast on change and on peer-connect; a query is answered by the account's owner. `GetJoinInfo` honours the header's LimitExceeded buffer contract.
- **Known gaps / deferred (tracked, not bugs):**
  ...
  - `EOS_Sessions_Register/UnregisterPlayers` callbacks do not yet populate the `RegisteredPlayers`/`SanctionedPlayers` out-arrays (games already hold the list they passed).
