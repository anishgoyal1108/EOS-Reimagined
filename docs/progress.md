...
- **Tests:** 196 unit cases / 11k+ assertions, integration (loads the real built lib), export-surface, and real-mesh e2e (2- and 3-instance) — green on both targets, clean under ASan + UBSan.
- **Review discipline:** each interface got an adversarial review (verify against headers + 2020 source, push back, fix with regression tests). Serious issues found and fixed this pass: P2P delivery/validation; networked-discovery identity; Sessions forged-verdict + reserve-amplification + search-drop; Presence forge/overwrite + silent-cap; Lobby Find-callback heap-overflow + lobby-state hijack + JoinById-timeout; plus the connection-bound-identity hardening.
- **Next (not started):** the Authenticated Mesh Identity milestone — self-certifying key-derived ids over a standards-exact Noise XX channel (Monocypher 4.0.3). Deferred by decision until after this freeze.
