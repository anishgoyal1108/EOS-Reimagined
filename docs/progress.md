...
- **Identity is self-asserted (known limitation, not a bug):** the mesh carries each peer's product-user-id in the envelope but does not cryptographically authenticate it (no backend, no crypto — by design). The owner/participant checks on sessions and the epic-id→peer binding on presence therefore bind accounts to sources among *cooperating* peers and stop accidental cross-talk, casual forgery, and buggy peers; they are not a defense against a peer that actively spoofs another's id. A presence binding is released when its peer disconnects so a departed account is free again. Hardening this further would require an authenticated identity handshake.
