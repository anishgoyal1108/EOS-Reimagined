...
- **A peer proves who it is before it is anyone.** Every connection runs Noise XX before it becomes a peer, and is adopted under the id *recomputed* from the key it proved. There is no field in which a peer says who it is, so first contact is no longer self-asserted. The Epic-account id is derived from the same key rather than travelling in a payload, so it can no longer be claimed either.
