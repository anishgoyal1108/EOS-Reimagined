---
title: "Reverse-Engineering Progress Manifest"
---
...
- [x] `architecture.qmd`, `protocol.qmd` written from source mapping
...
Self-certifying, key-derived identities over a standards-exact Noise XX channel. Spec: [`adr/0001`](adr/0001-authenticated-mesh-identity.qmd). This closes both gaps the foundation left open, and makes `EOS_EPacketReliability` mean something.
...
- **Complete compatibility surface** (`712a4b0`): 282 generated shells cover every declaration the
  vendored headers add beyond the 356 handwritten exports, including deprecated `.inl` declarations.
  The library now exports 638 `EOS_*` symbols, and all 613 entry points Risk of Rain 2 declares resolve.
  Async shells defer an `EOS_NotImplemented` callback instead of hanging; getters return empty values;
  output pointers are initialized; notifications return removable ids. These are compatibility
  answers, not implementations of Ecom, RTC, AntiCheat, Achievements, Stats, storage, or the other
  backend-dependent families.
- **Generated-shell tracing:** every generated export records its API version and honest return shape.
  Callback-bearing calls share one correlation across call, return, and deferred callback, including
  the non-void storage calls; absent transfer handles are explicit nulls, and notification return /
  register / remove records share one opaque label.
- **Handwritten ABI tracing:** bootstrap, common, logging, Platform, Connect, Auth, P2P, Sessions,
  Presence, Lobby, IntegratedPlatform, Friends, UserInfo, and UI are covered (all 356 handwritten exports). Calls
  rejected before dispatch remain visible; synchronous results use typed return values; asynchronous
  calls retain one correlation through callback
  delivery; returned notification ids use the same opaque labels as register/fire/remove records.
  P2P send traces contain endpoint/socket labels and packet metadata but never payload bytes.
- **Sessions ABI tracing and handle contracts:** all 64 exports are covered. Search, details,
  modification, and active-session outputs use opaque labels; invite ids, session names, attributes,
  addresses, and player ids never enter the trace. Invalid parent/sub-handles are rejected instead of
  being reported as `NotFound` or `Success`, and out-handles are null on failure.
- **Lobby ABI tracing and handle contracts:** all 79 exports are covered under the same rules.
  Deferred invite/RTC/overlay calls now reject a foreign parent instead of returning a plausible
  empty answer, while valid calls retain their honest `NotImplemented` or empty behavior.
- **Risk of Rain 2 LAN regressions (2026-07-16):** live Proton lobby evidence exposed two contract
  failures and each now has a red-before-green unit test. A held direct LobbyDetails handle now
  follows the live lobby after `UpdateLobby` commits instead of retaining a stale snapshot, stopping
  the game's repeated update loop. Connect mapping queries and synchronous external-account copies
  now expose one coherent stable Epic alias for every authenticated roster entry; a successful
  self-mapping query can no longer be followed by `CopyProductUserExternalAccountByIndex` returning
  `EOS_NotFound`, which had left Risk of Rain 2's avatar dictionary key null and retried every frame.
- **Real-run bundle privacy regression (2026-07-16):** the first manager bundle of that lobby run
  removed `display_name` but retained the user-selected instance label in `runtime.json` and all
  1,227,694 trace records. A red manager test now fixes the privacy aliasing at both surfaces:
  shareable metadata and trace `inst` fields use `instance#0`, while the non-secret configuration
  source origin remains. The packaged-CLI test also caught and prevents a double-newline introduced
  while canonicalizing records. The regenerated production bundle has one valid JSON record per line,
  matching per-file hashes and record counts, and no original instance label or absolute user path.
- **Two-process C-ABI probe:** two standalone workers load only the built `.so`/`.dll` and exercise
  the actual alpha path: separate persistent profiles, authenticated discovery, Friends/UserInfo,
  Sessions host/search/join, and negotiated P2P delivery. The coordinator cross-checks both traces by
  `peer_fp` and proves raw product ids and packet payloads are absent. It runs natively, under Wine,
  and with the sanitizer build.
- **Routed peer seeds:** optional bounded IPv4 `peer_seeds` / `EOSR_PEER_SEEDS` send the same
  untrusted discovery hint by unicast over the configured discovery-port range, while retaining LAN
  broadcast. This supports private routed overlays such as Tailscale without weakening the Noise
  identity boundary or claiming native Internet traversal. Runtime metadata records only the count.
- **Alpha runner and support bundle:** `tools/alpha_runner.py` owns one operational game run end to
  end: import census, unique run directory, recoverable hash-verified SDK staging, shell-free launch or
  supervised manual/Steam instructions, stdout capture, exact restoration, immutable `launch.json`,
  and an explicit sanitized bundle. The bundle excludes stdout/profile files, display names, command
  arguments, and absolute paths, and drops only a torn final trace record. Failure tests cover nonzero
  exit, start failure, post-replacement failure recovery, and refusal to overwrite an unrecognized DLL.
  An acceptance run against the installed 3 GB Risk of Rain 2 tree scanned 161 modules / 613 P/Invokes,
  found zero missing exports, staged the real game DLL, restored its exact pre-run SHA-256, and produced
  a path-clean bundle without leaving backup or transaction files behind.
- **Release-alpha pipeline:** the manual main-only release workflow builds and tests native Linux,
  MinGW/Wine, native MSVC, and ASan + UBSan before packaging. The Linux and Windows archives have a
  fixed membership, embedded `RELEASE.json`, outer manifest and SHA-256 list, deterministic metadata,
  and a verifier that rejects unsafe paths, duplicate members, missing files, and tampering. CTest
  reads `runtime.json` from the loaded artifact and requires the exact requested version and full
  commit. Publication is a GitHub prerelease targeted at that commit and refuses an existing tag or
  release; `publish=false` runs the entire pipeline as a retained rehearsal.
- **IntegratedPlatform stale handles:** flat calls now validate the interface against the live
  platform slot. A released or foreign handle returns `EOS_InvalidParameters` instead of continuing
  into a deinitialized object (or dereferencing an arbitrary pointer).
- **UI ABI tracing and stale handles:** all 27 calls are covered, with key/button combinations carried
  as numeric `flags` values. The flat layer now validates UI handles against the live platform too;
  the regression proves a handle retained past `EOS_Platform_Release` is rejected instead of reaching
  the deinitialized UI object.
...
- **Known gaps / deferred (tracked, not bugs):**
  ...
  - The three overlay-originated join notifications have no trigger yet: Presence
    ...
    callback on the game tick; see [companion-client.md](companion-client.qmd).
...
Detailed design and test matrix: [companion-client.md](companion-client.qmd).

## Milestone: all 42 classes across all 6 phases labeled + documented (2026-07-10).
- **Follow-ups resolved (2026-07-10):** full newer-build wire protocol recovered from embedded protobuf descriptors → `protocol.qmd` "RECOVERED newer-build schema". Confirmed UserInfo/Friends have NO network path (username via `EpicEmu_Infos_pb`); documented new `EpicAuth_pb`, `EpicCustomInvites_pb{payload}`, `EpicEmu_pb` handshake, `EpicOverlay_pb`; new fields (Presence `integratedplatform`, Lobby `rtc_room_name`, Sessions `owner_server_id`, search `app_id`).
