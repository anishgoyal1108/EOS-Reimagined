---
title: "Network Protocol & Transport"
---
...
- **UserInfo has no network path in this build** — a peer's display name/username arrives via `EpicEmu_Infos_pb.username` (the emu-info handshake) and `EpicPresence_pb`. (Resolves the `userinfo.qmd` open question.)
- **Friends has no network path** — derived entirely from the Connect roster via `OnUserEvent` (see `friends.qmd`).

## Emu-info handshake (`EpicEmu_pb`) — resolves `client.qmd` follow-up
