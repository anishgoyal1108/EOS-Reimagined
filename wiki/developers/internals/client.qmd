---
title: "Module: Bootstrap Infrastructure — `EOSSDK_Client` + `EOSSDK_Platform`"
---

Tier A (2020 source: `eos_client_api.*`, `eossdk_platform.*`, `eos_flat_platform.cpp`). Covers global SDK init, the platform singleton, interface ownership, tick, and the emu's own peer-info handshake. See also `architecture.qmd` (async engine) and `protocol.qmd`.
...
Infra classes have no EOS async API of their own; they host the machinery documented in `architecture.qmd`. `EOSSDK_Client::CBRunFrame` participates in the frame loop (emu-info housekeeping); `OnNetworkMessage` is the top-level inbound entry that routes to per-interface handlers.
...
### Follow-ups (open)
- Read `_SendEmuInfosRequest`/`_On*` bodies → document the emu-info wire message in `protocol.qmd`.
