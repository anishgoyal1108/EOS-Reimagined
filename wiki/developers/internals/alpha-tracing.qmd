...
5. Instrument one vertical slice first — `EOS_Initialize` → `EOS_Platform_Create` → one asynchronous
   ...
   - **Also done:** 106 handwritten bootstrap, common, logging, Platform, Connect, and Auth exports.
     The loaded-library test covers real and rejected calls, typed returns, async correlation, and a
     notification id shared with its register/fire/remove records.
   - **Still to do:** the remaining 250 handwritten interface exports using the same trampoline
     pattern.
