...
**alpha runner** — so it is defined once here and referenced everywhere else. The complete pipeline
is implemented; its acceptance coverage and next in-game step are tracked in §11.
...
means one file everywhere. `data_dir` is resolved from `EOSR_DATA_DIR`, a valid
[sibling bootstrap descriptor](adr/0002-bootstrap-descriptor.qmd), or the platform default *before*
the file is read, so locating the file never depends on the file; `data_dir` is the one field the
file cannot set.
...
| Data directory | *(n/a)* | `EOSR_DATA_DIR` | bootstrap descriptor, then platform default | Profile/key storage. The descriptor is manager-owned and never overrides a non-empty environment value. |
...
| Peer seeds | `peer_seeds` | `EOSR_PEER_SEEDS` | `[]` | Up to 16 unicast IPv4 literals; JSON string array or comma-separated env. Uses the resolved discovery-port range. |
...
```json
{
  ...
  "peer_seeds": ["100.80.37.76", "100.91.2.3"],
...
- **Peer seeds** — at most 16 unique unicast IPv4 literals. Resolution is atomic per layer and
  rejects hostnames, embedded ports, multicast/unspecified addresses, and leading-zero forms.
...
```json
{
  ...
  "config": {
    ...
    "trace_level": "lifecycle",
    "log_level": "info",
    "trace_dir": "/home/alice/.local/share/eos-reimagined/traces",
    ...
    "sources": {
      ...
      "trace_dir": "default",
      "trace_max_bytes": "default",
      "trace_max_rotated_files": "default",
      "discovery_ports": "file",
      "peer_seeds": "file",
      "instance_label": "file",
      "enable_lan": "default",
      "enable_overlay": "default",
      "unlock_dlcs": "default"
    }
  }
}
...
profile. Seed addresses never enter runtime or trace output; only their count is recorded. Fields the
writer cannot determine are `null`, never omitted, so the shape is stable.

Every manager-editable directive has a source of `environment`, `file`, or `default`, recorded only
after invalid higher-priority values have fallen through. This is authoritative evidence for why the
running value differs from `eosr.json`. Older schema-1 files that predate `sources` remain readable;
the manager labels their provenance unavailable instead of guessing.
...
```json
{
  ...
  "artifact": { "path": ".../EOSSDK-Win64-Shipping.dll", "sha256": "<ours>",
                "replaced_original_sha256": "<the game's Epic DLL>", "restored": true },
  "launch": { "argv_redacted": ["<game>", "<arguments-redacted>"], "cwd": "...",
              "exit_code": 0, "status": "exited" }
}
```

Arguments are never persisted: only the executable basename and the fact that arguments existed are
recorded. A manual launcher has `argv_redacted: ["<manual>"]` and a null exit code. `restored` records
whether the verified original was back in place when the runner finalized the file.
...
`stdout.log` is deliberately not copied: arbitrary game output has no structural redaction boundary.
The summary carries only its byte count and SHA-256 so support can tell whether output existed and
whether two local copies match. Trace files are copied record by record; a single non-newline-terminated
tail is dropped, while an invalid complete record rejects the bundle rather than laundering an unknown
format into something labelled shareable. Profile keys and every other unowned file are ignored.
...
- **Runner restoration** — a normal exit, nonzero exit, failed process start, interrupt, and failure
  after replacement all restore the hash-verified original; an unrecognized file is never overwritten.
- **Bundle boundary** — stdout, profile keys, owned-file free text, command arguments, and a torn trace
  tail do not enter the shareable directory.
...
- **C-ABI probe (implemented)** — two processes, distinct `data_dir`s, loading the real `.so`/`.dll`
  and calling only exported EOS functions. It proves authenticated discovery, Friends/UserInfo,
  Sessions host/search/join, and P2P delivery natively and under Wine, and produces the same owned
  files and trace a game run does.
...
- **Alpha runner (implemented)** — `tools/alpha_runner.py` selects the artifact, runs the import census,
  creates `EOSR_RUN_DIR`, stages under a recoverable hash-verified transaction, launches without a
  shell (or prints and supervises exact manual/Steam steps), captures stdout/stderr locally, restores
  the original, writes `launch.json`, and builds the explicit sanitized bundle. `restore` recovers a
  transaction left by a killed runner; it refuses to overwrite a file that is neither the recorded
  original nor the staged artifact.
...
5. Instrument one vertical slice first — `EOS_Initialize` → `EOS_Platform_Create` → one asynchronous
   ...
   - **Also done:** all 356 handwritten bootstrap, common, logging, Platform, Connect, Auth, P2P,
     Sessions, Presence, Lobby, IntegratedPlatform, Friends, UserInfo, and UI exports.
     ...
     fabricated delivery.
     Sessions search/sub-handle returns carry opaque labels, and its dormant
     `JoinSessionAccepted` hook has the same notification-label continuity.
     Lobby search/sub-handle returns and the dormant `JoinLobbyAccepted` hook follow the same
     contract.
     UI key/button bitmasks use the numeric `flags` return type rather than being mislabeled as counts.
   - **The complete 638-export ABI surface is now observable.**
6. **Done:** the two-process C-ABI probe is the first full consumer. Its coordinator joins the two
   traces by peer fingerprint, rejects raw-id or payload leakage, and runs under Linux, Wine, and the
   sanitizer build.
7. **Done:** the alpha runner and sanitized support bundle. Unit coverage exercises unique SDK
   selection, exact staging/restoration, post-replacement failure recovery, nonzero and failed launches,
   path/display-name/argument redaction, stdout/profile exclusion, and torn-tail handling. The next step
   is a real two-machine Risk of Rain 2 LAN run, using the bundle to turn the first behavioral mismatch
   into a reproducible regression.
