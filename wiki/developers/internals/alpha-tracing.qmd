...
factories (`return_bool`, `return_count`, `return_handle`, `return_null_handle`, `return_enum`,
...
5. Instrument one vertical slice first — `EOS_Initialize` → `EOS_Platform_Create` → one asynchronous
   ...
   - **Also done:** 186 handwritten bootstrap, common, logging, Platform, Connect, Auth, P2P,
     Presence, IntegratedPlatform, Friends, and UserInfo exports.
     ...
     fabricated delivery.
   - **Still to do:** the remaining 170 handwritten interface exports using the same trampoline
     pattern.
