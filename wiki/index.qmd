...
::: {.hero-buttons}
[Get started](users/getting-started.qmd){.btn .btn-primary}
[What works today](users/roadmap.qmd){.btn .btn-outline-secondary}
...
::: {.grid}
::: {.g-col-12 .g-col-md-6}
::: {.feature-card .lead-card}
[🎮]{.card-icon}

### [For users](users/index.qmd){.stretched-link}

You want a game set up and playing. The walkthrough, the `eosr.json` configuration reference,
identity & profiles, firewall rules, and the troubleshooting that fixes the usual suspects.

[Getting started · Configuration · Networking · Troubleshooting]{.lead-card-trail}
:::
:::
...
### [For developers](developers/index.qmd){.stretched-link}

You want to hack on it. The contribution rules, the repository map, and the full
reverse-engineered behavioral spec the implementation is built from — protocol, ADRs, and one
page per EOS interface.

[Contributing · Internals · Progress manifest · Export census]{.lead-card-trail}
:::
:::
:::
...
[scope & non-goals](users/scope.qmd).
