...
net_envelope make_p2p_envelope(message_type type, const std::string& source, const std::string& socket,
...
TEST_CASE("receiving into too small a buffer reports LimitExceeded without consuming") {
    p2p_fixture fx;
    ...
    // The packet was not consumed; GetNextReceivedPacketSize still reports it.
    EOS_P2P_GetNextReceivedPacketSizeOptions size_options = {};
    size_options.ApiVersion = EOS_P2P_GETNEXTRECEIVEDPACKETSIZE_API_LATEST;
    size_options.LocalUserId = fx.local();
    u32 size = 0;
...
TEST_CASE("accepting a connection fires the established notification") {
    p2p_fixture fx;
